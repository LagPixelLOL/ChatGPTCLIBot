...
namespace config {
     ...
     */
    void Config::load_config(const std::function<void(const log& msg)>& log_callback) {
        log_callback(log{log_level::INFO, "Loading config file...", config_path});
        if (!file::exists(config_path)) {
            log_callback(log{log_level::ERR, "Config file does not exist, creating new config file..."});
            save_config();
            return;
        }
                ...
                if (it_api_key->is_string()) {
                    ...
                    for (const auto& key : *it_api_key) {
                        if (key.is_string()) {
                            ...
                            log_callback(log{log_level::ERR, "Argument api_key has non-string element. Element: " + key.dump(2)});
                            error = true;
                        }
                    }
                    api::set_key(keys);
                } else {
                    log_callback(log{log_level::ERR, "Argument api_key is not a string, array or null."});
                    error = true;
                }
            }
        } else {
            log_callback(log{log_level::ERR, "Argument api_key is not found."});
            error = true;
        }
        ...
        if (it_model != j.end() && it_model->is_string()) {
            ...
            log_callback(log{log_level::ERR, "Argument model is not a string."});
            error = true;
        }
        ...
        if (it_temperature != j.end() && it_temperature->is_number()) {
            ...
            log_callback(log{log_level::ERR, "Argument temperature is not a number."});
            error = true;
        }
        ...
        if (it_max_tokens != j.end() && it_max_tokens->is_number_integer()) {
            ...
            if (max_tokens <= 0) {
                log_callback(log{log_level::ERR, "Argument max_tokens is less or equal to 0."});
                max_tokens = 1;
                error = true;
            }
        } else {
            log_callback(log{log_level::ERR, "Argument max_tokens is not an integer number."});
            error = true;
        }
        ...
        if (it_top_p != j.end() && it_top_p->is_number()) {
            ...
            log_callback(log{log_level::ERR, "Argument top_p is not a number."});
            error = true;
        }
        ...
        if (it_frequency_penalty != j.end() && it_frequency_penalty->is_number()) {
            ...
            log_callback(log{log_level::ERR, "Argument frequency_penalty is not a number."});
            error = true;
        }
        ...
        if (it_presence_penalty != j.end() && it_presence_penalty->is_number()) {
            ...
            log_callback(log{log_level::ERR, "Argument presence_penalty is not a number."});
            error = true;
        }
        ...
        if (it_logit_bias != j.end() && it_logit_bias->is_object()) {
            ...
            for (const auto& [key, value] : it_logit_bias->items()) {
                if (!value.is_number()) {
                    log_callback(log{log_level::ERR, "Argument logit_bias has non-number bias. Value: " + value.dump(2)});
                    error = true;
                    continue;
                }
                float bias = value.get<float>();
                if (bias < -100 || bias > 100) {
                    log_callback(log{log_level::ERR, "Argument logit_bias's bias is out of range, it must be between -100 and 100. Bias: "
                    + std::to_string(bias)});
                    error = true;
                    continue;
                }
            ...
            log_callback(log{log_level::ERR, "Argument logit_bias is not an object."});
            error = true;
        }
        ...
        if (it_max_display_length != j.end() && it_max_display_length->is_number_unsigned()) {
            ...
            if (max_display_length == 0) {
                log_callback(log{log_level::ERR, "Argument max_display_length cannot be 0."});
                max_display_length = 1;
                error = true;
            }
        } else {
            log_callback(log{log_level::ERR, "Argument max_display_length is not an unsigned integer."});
            error = true;
        }
        ...
        if (it_max_short_memory_length != j.end() && it_max_short_memory_length->is_number_unsigned()) {
            ...
            if (max_short_memory_length == 0) {
                log_callback(log{log_level::ERR, "Argument max_short_memory_length cannot be 0."});
                max_short_memory_length = 1;
                error = true;
            }
        } else {
            log_callback(log{log_level::ERR, "Argument max_short_memory_length is not an unsigned integer."});
            error = true;
        }
        ...
        if (it_max_reference_length != j.end() && it_max_reference_length->is_number_unsigned()) {
            ...
            log_callback(log{log_level::ERR, "Argument max_reference_length is not an unsigned integer."});
            error = true;
        }
        ...
        if (it_search_response != j.end() && it_search_response->is_boolean()) {
            ...
            log_callback(log{log_level::ERR, "Argument search_response is not a boolean."});
            error = true;
        }
        ...
        if (it_space_between_exchanges != j.end() && it_space_between_exchanges->is_boolean()) {
            ...
            log_callback(log{log_level::ERR, "Argument space_between_exchanges is not a boolean."});
            error = true;
        }
        ...
        if (it_debug_reference != j.end() && it_debug_reference->is_boolean()) {
            ...
            log_callback(log{log_level::ERR, "Argument debug_reference is not a boolean."});
            error = true;
        }
#ifdef __linux__
        ...
        if (it_ca_bundle_path != j.end() && it_ca_bundle_path->is_string()) {
            ...
            log_callback(log{log_level::ERR, "Argument ca_bundle_path is not a string."});
            error = true;
        }
#endif
        if (error) {
            log_callback(log{log_level::INFO, "Error detected, creating new config file..."});
            save_config(log_callback);
            return;
        }
        log_callback(log{log_level::INFO, "Config loaded."});
    }

    /**
     * @throw std::exception If an error occurred when saving the config file.
     */
    void Config::save_config(const std::function<void(const log& msg)>& log_callback) {
        log_callback(log{log_level::INFO, "Saving config to file...", config_path});
        nlohmann::json j;
        ...
        log_callback(log{log_level::INFO, "Config saved."});
    }
    //class Config end.
} // config