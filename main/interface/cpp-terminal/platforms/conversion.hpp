...
namespace Term::Private {
    ...
    inline void codepoint_to_utf8(std::string& s, char32_t c)
    {
        ...
        {
            nbytes++;
            bytes[3] = (char)((d | 0x80) & 0xBF);
            d >>= 6;
        }
        if(c >= 0x800)
        {
            nbytes++;
            bytes[2] = (char)((d | 0x80) & 0xBF);
            d >>= 6;
        }
        if(c >= 0x80)
        {
            nbytes++;
            bytes[1] = (char)((d | 0x80) & 0xBF);
            d >>= 6;
        }
    ...
    inline std::u32string utf8_to_utf32(const std::string& s) {
        ...
        for (char i : s) {
            ...
            if (state == UTF8_ACCEPT) {
                ...
                throw Term::Exception("Invalid byte in UTF8 encoded string");
            }
        }
        if (state != UTF8_ACCEPT) {
            throw Term::Exception("Expected more bytes in UTF8 encoded string");
        }
        return r;
    }
